...
With those commands I was able to see that in my case the Immich database was split between `disk2` and `cache` and in some instances the Immich server accessed the cache, but in others - `disk2`, which naturally woke up the disk.
